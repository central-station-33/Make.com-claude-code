/**
 * list-pending-enrichment — returns the next batch of ISA leads needing AI
 * enrichment, in segment-priority order, each with its ready-to-send prompt
 * text. Used by the Gemini-primary enrichment pass (S2): Make fetches this
 * list, loops it through the Gemini AI module using its own connection, then
 * posts each result to write-enrichment. enrich-leads (Anthropic) runs after
 * as the backup pass — it only touches leads still missing ai_summary, so
 * anything Gemini already wrote is automatically skipped.
 *
 * POST body: { limit?: number (default 30), segment?: string }
 */
...
// Kept in sync with enrich-leads — both must score/format the same way so a
// lead reads identically to an ISA regardless of which model produced it.
...
serve(async (req) => {
  ...
  const leads = rows.map((lead) => ({ id: lead.id, prompt: buildPrompt(lead) }));
